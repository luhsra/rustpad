...
use crate::collab::connection::Connection;
...
impl AxumConn {
    pub fn new(awareness: Arc<Awareness>, socket: WebSocket) -> Self {
...
/// A websocket sink wrapper, that implements futures `Sink` in a way, that makes it compatible
...
/// A websocket stream wrapper, that implements futures `Stream` in a way, that makes it compatible
/// with y-sync protocol, so that it can be used by y-sync crate [BroadcastGroup].
#[derive(Debug)]
    ...
    fn poll_next(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        match Pin::new(&mut self.0).poll_next(cx) {
            ...
            Poll::Ready(Some(res)) => match res {
                Ok(item) => Poll::Ready(Some(Ok(item.into_data().into()))),
...
mod test {
    ...
    async fn start_server(
        ...
        Ok((
            tokio::spawn(async move { axum::serve(listener, ws).await.unwrap() }),
            addr,
        ))
    }
    ...
    async fn client(
        ...
        let (stream, _) = tokio_tungstenite::connect_async(format!("ws://{addr}/my-room")).await?;
    ...
    async fn change_introduced_by_server_reaches_subscribed_clients() {
        ...
        let (_server, addr) = start_server(Arc::new(bcast)).await.unwrap();

        let doc = Doc::new();
        let (n, _sub) = create_notifier(&doc);
        let _c1 = client(addr, doc).await.unwrap();
    ...
    async fn subscribed_client_fetches_initial_state() {
        ...
        let doc = Doc::new();
        let (n, _sub) = create_notifier(&doc);
        let c1 = client(addr, doc).await.unwrap();
    ...
    async fn changes_from_one_client_reach_others() {
        ...
        let c1 = client(addr, d1).await.unwrap();
        ...
        let c2 = client(addr, d2).await.unwrap();
    ...
    async fn client_failure_doesnt_affect_others() {
        ...
        let c3 = client(addr, d3).await.unwrap();
