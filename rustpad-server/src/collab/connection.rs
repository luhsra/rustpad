    ...
    E: Into<Error> + Send + Sync,
{
        ...
        P: Protocol + Send + Sync + 'static,
    {
        ...
        // Wrap awareness in SendSyncAwareness for thread safety in async blocks
                ...
                match input {
                    Ok(data) => {
                        if let Some(mut sink) = loop_sink.upgrade() {
                            if let Some(awareness_wrapper) = loop_awareness.upgrade() {
                                match Self::process(&protocol, &awareness_wrapper, &mut sink, data)
                                    .await
                                {
                                    Ok(()) => { /* continue */ }
                                    Err(e) => {
...
mod test {
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        let (_server, server_addr) = start_server(bcast).await?;
